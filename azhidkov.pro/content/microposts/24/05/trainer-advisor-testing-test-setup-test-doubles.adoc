...
Это четвёртый пост о тестировании link:++{{<ref "/microposts/23/11/qyoga">}}++[Trainer Advisor] и третий (и последний) пост о сетапе фикстуры.

Ранее в сериале:

. link:++{{<ref "/microposts/24/03/trainer-advisor-testing-theory">}}++[Теория и общие идеи подхода к тестированию];
. link:++{{<ref "/microposts/24/04/trainer-advisor-testing-test-suite-setup">}}++[Запуск инфраструктуры и системы для тестов];
. link:++{{<ref "/microposts/24/04/trainer-advisor-testing-test-setup-generate-fixture">}}++[Генерация и вставка фикстурных данных в хранилища];
...
Как я писал в link:++{{<ref "/microposts/24/04/trainer-advisor-testing-test-suite-setup#_запуск_wiremock">}}++[посте о сетапе инфраструктуры], тестирование кода, работающего с внешними сервисами, у меня проработано намного меньше, чем тестирование кода, работающего с БД, но, тем не менее, кое-какие наработки есть.
