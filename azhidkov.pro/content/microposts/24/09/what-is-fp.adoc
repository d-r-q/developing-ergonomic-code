...
Плюс, я полагаю, кто-то из других оппонентов под ФП имеет ввиду монады и Haskell с сигнатурами вида `loadUser(): Result<Future<Try<User>>>`, а кто-то трансдюсеры и Clojure где вместо циклов только хвостовая рекурсия.
...
. Относительно помещения её в класс ApplyDiscount - гарантию того что, там не случайно не появится ввод-вывод;
. Относительно помещения её в класс Order:
.. спасение Order от новой зависимости на Program - сохранение никого coupling-а
...
. Рубеж обороны от переусложнения операции - на уровне гайдлайна запретить добавление 8-ой зависимости в конструктор без созыва архитектурного комитета.

Но если вам такой подход не нравится - ради бога, используйте вариант из fDDD.
Только делайте модель в record-ах, и бизнес-логику - в чистых функциях.
