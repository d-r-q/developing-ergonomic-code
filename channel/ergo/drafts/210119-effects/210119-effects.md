...
*Привет!*
*Сегодня подготовил вам лонгрид на 1.5К слов - обычно на 500.*
*Если вы думаете, что его будет долго читать, то представьте, сколько его было писать:) 8 часов, я замерял:)*
...
*Поэтому прошу вас дать обратную связь по этому посту, можно в личку, если в комменты стрёмно.*
...
> Задачка
>
> Вообще мне бы хотелось оставить слово Эффект для записи и Обработка Сигнала для чтения, но я чёт не могу придумать каких словом их объединить. Есть идеи?
...
Поэтому в тест-планах всегда есть шаги (набор сигналов для отправки) и ожидаемый результат (набор сигналов для проверки).
А в случае информационных систем 90-99% тест кейсов вообще не содержат каких-либо правил по проверке ожидаемого результата, т.к. ожидаемый результат бинарный - логин удался или нет, введённая строка появилась в нужном месте или нет.
...
2. на [как минимум на порядок, а то и пять](https://formulusblack.com/blog/compute-performance-distance-of-data-as-a-measure-of-latency/) медленнее тестов только в памяти
